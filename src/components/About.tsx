      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="mt-6 text-dark-400 text-lg leading-relaxed">
          ...
          >
            {metrics.map((m, i) => (
              <div
                key={m.label}
                className={`p-6 rounded-2xl border border-dark-100 ${
                  i === 0 ? "bg-dark-950 border-dark-950" : "bg-white"
                }`}
              >
                <m.icon
                  className={`w-7 h-7 mb-4 ${
                    i === 0 ? "text-accent-400" : "text-primary-500"
                  }`}
                />
                <p
                  className={`text-2xl font-bold ${
                    i === 0 ? "text-white" : "text-dark-950"
                  }`}
                >
                  {m.value}
                </p>
                <p
                  className={`text-sm mt-1 ${
                    i === 0 ? "text-white/60" : "text-dark-400"
                  }`}
                >
                  {m.label}
                </p>
